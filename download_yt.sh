...
yt-dlp -F "$url"
echo -e "${BLUE}📊 Enter the format code (e.g., 137+140 for 1080p+Audio, or 'best' for automatic):${NC}"
read -rp "> " format_code
...
if [[ "$want_subs" == "y" || "$want_subs" == "Y" ]]; then
    ...
    echo -e "${BLUE}✍️ Enter the subtitle language code (e.g., en, ar) or type 'all':${NC}"
    read -rp "> " sub_lang
    
    if [ "$sub_lang" == "all" ]; then
        dl_args="$dl_args --write-sub --all-subs"
    elif [ -n "$sub_lang" ]; then
        dl_args="$dl_args --write-sub --sub-lang $sub_lang"
    fi
fi
