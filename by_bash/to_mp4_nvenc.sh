...
echo -e "${YELLOW}⏳ Converting to MP4 using NVIDIA NVENC...${NC}"

ffmpeg -hwaccel cuda -hide_banner -loglevel error -stats -i "$input_file" -c:v h264_nvenc -c:a aac -movflags +faststart "$output_file"
