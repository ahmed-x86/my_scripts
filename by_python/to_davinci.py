...
def get_video_stats(input_file):
    """Fetch video duration and total frame count"""
    duration = 0.0
    total_frames = 0
    ...
    try:
        ...
        # If total frames is already recorded
...
def main():
    ...
    output_file = f"{name}_davinci.mov"

    print(f"{YELLOW}⏳ Reading video data...{NC}")
    duration, total_frames = get_video_stats(input_file)

    print(f"{YELLOW}⏳ Converting to DaVinci Resolve format (DNxHR)...{NC}\n")
    send_notification("Video Converter", f"Starting conversion for {input_file}...", "video-x-generic")

    # Run ffmpeg with DNxHD/HR and uncompressed audio parameters
    cmd = [
    ...
    # Regex patterns to extract FFMPEG data
    ...
    for line in process.stderr:
        ...
        if time_match and frame_match:
            # Extract current time and frames
