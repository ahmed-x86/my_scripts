...
def main():
    ...
    output_file = f"{name}_prores_cuda.mov"

    print(f"{YELLOW}⏳ Reading video data...{NC}")
    duration, total_frames = get_video_stats(input_file)

    print(f"{YELLOW}⏳ Converting to ProRes 422 with CUDA Decoding...{NC}\n")
    send_notification("Video Converter", f"Starting CUDA-assisted ProRes conversion for {input_file}...", "video-x-generic")

    # Run ffmpeg with CUDA decoding and ProRes 422 parameters
    cmd = [
