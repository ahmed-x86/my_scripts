...
def main():
    ...
    output_file = f"{name}_prores_amd_full.mov"

    print(f"{YELLOW}⏳ Reading video data...{NC}")
    duration, total_frames = get_video_stats(input_file)

    print(f"{YELLOW}⏳ Processing with AMD VA-API Pipeline (ProRes 422)...{NC}\n")
    send_notification("Video Converter", f"Starting full AMD pipeline for {input_file}...", "video-x-generic")

    # Run ffmpeg with full AMD VA-API hardware acceleration pipeline
    # Download the surface from GPU to CPU, format to nv12, then to yuv422p10le for 10-bit ProRes
    cmd = [
        "ffmpeg", "-y", 
    ...
    if process.returncode == 0:
        ...
        send_notification("Success", f"AMD Pipeline finished: {output_file}", "dialog-information")
    else:
