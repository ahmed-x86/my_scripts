...
def main():
    ...
    print(f"{YELLOW}⏳ Cleaning audio and saving to MP4...{NC}")

    duration = get_duration(input_file)
    
    # Run ffmpeg with audio filtering, loudnorm, and video copy
    cmd = [
        "ffmpeg", "-y", "-i", input_file, 
        "-c:v", "copy", 
        "-af", "afftdn=nf=-25,loudnorm=I=-16:LRA=7:TP=-1.5", 
        "-c:a", "aac", "-b:a", "192k", 
        output_file
    ]
    ...
    if process.returncode == 0:
        print(f"{GREEN}✅ Audio cleaning completed successfully: {output_file}{NC}")
    else:
