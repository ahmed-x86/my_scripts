...
def main():
    ...
    output_file = f"{name}_mjpeg_intel.mov"

    print(f"{YELLOW}⏳ Reading video data...{NC}")
    duration, total_frames = get_video_stats(input_file)

    print(f"{YELLOW}⏳ Converting with Intel Iris Xe (VA-API) Decoding...{NC}\n")
    send_notification("Video Converter", f"Starting Intel VA-API conversion for {input_file}...", "video-x-generic")
    ...
    if process.returncode == 0:
        ...
        send_notification("Success", f"Intel VA-API conversion finished: {output_file}", "dialog-information")
    else:
