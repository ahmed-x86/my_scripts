...
def main():
    ...
    output_file = f"{name}_prores_amd.mov"

    print(f"{YELLOW}⏳ Converting to ProRes 422 with AMD VA-API Decoding...{NC}")
    send_notification("Video Converter", f"Starting AMD-assisted ProRes conversion for {input_file}...", "video-x-generic")

    duration = get_duration(input_file)
    
    # Run ffmpeg with AMD VA-API decoding and ProRes 422 parameters
    cmd = [
