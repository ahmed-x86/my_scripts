...
def main():
    ...
    print(f"{YELLOW}⏳ Processing with CUDA Pipeline (ProRes 422)...{NC}\n")
    send_notification("Video Converter", f"Starting full CUDA ProRes pipeline for {input_file}...", "video-x-generic")

    # Run ffmpeg with full CUDA hardware acceleration pipeline
    # Download the frame from VRAM to RAM and format to yuv422p10le for 10-bit ProRes
    cmd = [
        "ffmpeg", "-y", 
        "-hwaccel", "cuda", 
    ...
    if process.returncode == 0:
        ...
        send_notification("Success", f"CUDA Pipeline finished: {output_file}", "dialog-information")
    else:
