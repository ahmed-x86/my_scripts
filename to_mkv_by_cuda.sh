...
echo -e "${YELLOW}⏳ Converting to MKV with CUDA Decoding...${NC}"

ffmpeg -hwaccel cuda -hide_banner -loglevel error -stats -i "$input_file" -c:v libx264 -c:a aac "$output_file"
