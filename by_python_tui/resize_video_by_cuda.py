...
def main_tui(stdscr):
    ...
    idx = prompt_choice(stdscr, scale_opts, "📏 Select Scaling Percentage (Hybrid CUDA)")
    sel = scale_opts[idx]
    ...
    output_file = f"{name}_{sel['pct']}percent_cuda{ext}"
    
    cmd = ["ffmpeg", "-y", "-hwaccel", "cuda", "-i", input_file, "-vf", f"scale=trunc(iw*{sel['scale']}/2)*2:trunc(ih*{sel['scale']}/2)*2", "-c:v", encoder, "-c:a", "copy", output_file]
    run_ffmpeg_ui(stdscr, cmd, duration, frames, f"Resizing (CUDA Decode -> CPU Encode) to {sel['pct']}%...")
